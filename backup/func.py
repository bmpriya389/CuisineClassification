...
def create_dataset(db):
    ...
    ingredients_global['cuisine'] = training_dataset['cuisine']

    ingredients_count = ingredients_global.to_dict(orient = 'records')
    ...
    try:
        ...
        map(lambda y: db.ingredientCount.insert_one(y), ingredients_count)
...
def predictor_score(ingredients,db):
    ...
    X = pandas.DataFrame([i for i in db.ingredientCount.find()])
    y=X['cuisine']
    del X['_id']
    del X['cuisine']
    model.fit(X=X, y=y)
